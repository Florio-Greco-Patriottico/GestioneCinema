...
public class AbstractProduct implements IProduct {
	...
	private int    quantity;						//variabile per la quantità
	...
	public void setQuantity(int q) {
		//controllo se il parametro è valido
		if(q > 0) {
			//controllo se vi è già una quantità
			if(this.quantity > 0) {
	...
	public void setPrice(double p) {
		//controllo se il prezzo è maggiore di 0
		if(p > 0.0) {
			this.price = p;
			//notifico
	...
	public void setDescription(String d) {
		try {
			this.description = d;
			//notifico
			System.out.println("La descrizione del prodotto è stata cambiata in: " + getDescription() );
		}catch(Exception e) {
	...
	public void setName(String n) {
		try {
			this.name = n;
			//notifico
			System.out.println("Il nome del prodotto è stato cambiato in: " + getName() );
		}catch(Exception e) {
			System.out.println(e);
		}
	}
